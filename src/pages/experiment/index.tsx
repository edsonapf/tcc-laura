...
import {
  ...
  QuestionResponse,
  TrialRequest,
} from "@/interfaces/ExperimentData";
import { OnFinish, Trial } from "@/interfaces/OnFinish";
import { initJsPsych } from "jspsych";
import { useEffect, useRef, useState } from "react";
import { useNavigate } from "react-router";

const experiments = EXPERIMENTS.sort(() => Math.random() - 0.5);
...
export default function Experiment() {
  ...
  const handleClick = (correct: boolean) => {
    const isFake = experiments[experimentIndex].fake;
    const questionResponse: QuestionResponse = {
      correct,
      phrase: experiments[experimentIndex].phrase,
      timeElapsed: Date.now() - question.start,
    };
    ...
    setCurrentPhrasePosition((prev) => prev + 1);
  };

  return question.show ? (
    <Question
      answers={experiments[experimentIndex].answers}
      finishText="Finalizar pesquisa"
